...
describe("API Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    // Initialize database. resetState() now wipes to a blank admin-only state
    // (no demo re-seed), so rebuild the demo dataset explicitly.
    await initDatabase();
    await resetState();
    ...
    app.get("/api/state", (req, res) => {
      ...
      res.json({
        ...
        notifications: state.notifications.filter(
          (n) => n.userId === viewer.id,
        ),
        timeline: state.timeline,
        auditLogs: viewer.role === "BRANCH_USER" ? [] : state.auditLogs,
        currentUser: { ...viewer, passwordHash: undefined },
      });
    });
    ...
    app.post("/api/tickets", async (req, res) => {
      ...
      if (!subject || !description || !category) {
        return res
          .status(400)
          .json({ error: "subject, description, and category are required" });
      }
    ...
    app.post("/api/users", async (req, res) => {
      ...
      if (!username || !name || !role || !email) {
        return res
          .status(400)
          .json({ error: "username, name, role, and email are required" });
      }
      state = createUser(
        state,
        {
    ...
    app.post("/api/branches", async (req, res) => {
      ...
      if (!name || !location)
        return res
          .status(400)
          .json({ error: "branch name and location are required" });
      state = createBranch(
        state,
        { name, location, code, status: "Active" },
        viewer,
      );
  ...
  afterAll(async () => {
    // Leave the shared database clean: wipe back to the blank admin-only state
    // so running the tests never leaves demo data behind in production.
    await resetState();
    await closeDatabase();
  });
  ...
  describe("State Access", () => {
    ...
    it("GET /api/state filters tickets for branch user", async () => {
      ...
      tickets.forEach((ticket: Ticket) => {
        expect(
          ticket.branchId === branchUser.branchId ||
            ticket.requesterId === branchUser.id,
        ).toBe(true);
      });
    });
  });
