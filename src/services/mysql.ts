...
export async function initDatabase(): Promise<void> {
  pool = createPool();
  await pool.query(`CREATE DATABASE IF NOT EXISTS \`${dbName}\` CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci`);
  await pool.end();
  ...
  try {
    ...
    for (const c of INITIAL_COMMENTS) {
      await conn.query(
        ...
        [c.id, c.ticketId, c.authorId, c.authorName, c.authorRole, c.content, c.timestamp, c.isInternal ? 1 : 0]
      );
    }
    ...
    for (const t of INITIAL_TIMELINE) {
      await conn.query(
        ...
        [t.id, t.ticketId, t.timestamp, t.actorName, t.actorRole, t.action, t.details ?? null, t.type]
      );
    }
    ...
    for (const n of INITIAL_NOTIFICATIONS) {
      await conn.query(
        ...
        [n.id, n.userId, n.ticketId, n.title, n.message, n.timestamp, n.read ? 1 : 0, n.type]
      );
    }
    ...
    for (const a of INITIAL_AUDIT_LOGS) {
      await conn.query(
        ...
        [a.id, a.timestamp, a.actorName, a.actorRole, a.action, a.targetId ?? null, a.details]
      );
    }
...
export async function loadState(): Promise<AppState> {
  ...
  return {
    ...
    branches,
    categories,
    currentUser: parsedUsers.find((u) => u.id === currentUserId) || parsedUsers[0] || createState().currentUser,
    ticketCounter,
  };
}
...
export async function saveState(state: AppState): Promise<void> {
  ...
  try {
    ...
    await conn.query(
      'INSERT INTO app_meta (meta_key, meta_value) VALUES (?, ?), (?, ?) ON DUPLICATE KEY UPDATE meta_value = VALUES(meta_value)',
      ['currentUserId', state.currentUser.id, 'ticketCounter', String(state.ticketCounter)]
    );
...
const rowsEqual = (a: unknown, b: unknown): boolean => JSON.stringify(a) === JSON.stringify(b);
...
async function insertTimelineRow(conn: Queryable, t: TimelineEvent): Promise<void> {
  await conn.query(
    'INSERT INTO timeline (id, ticketId, timestamp, actorName, actorRole, action, details, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
    [t.id, t.ticketId, t.timestamp, t.actorName, t.actorRole, t.action, t.details ?? null, t.type]
  );
...
async function upsertNotificationRow(conn: Queryable, n: NotificationItem): Promise<void> {
  await conn.query(
...
async function setMetaVal(conn: Queryable, key: string, value: string): Promise<void> {
  await conn.query(
    'INSERT INTO app_meta (meta_key, meta_value) VALUES (?, ?) ON DUPLICATE KEY UPDATE meta_value = VALUES(meta_value)',
    [key, value]
  );
 ...
 */
export async function persistDiff(before: AppState, after: AppState): Promise<void> {
  const conn = await pool.getConnection();
  try {
    await conn.beginTransaction();
    ...
    for (const c of after.comments) {
      if (!before.comments.some((x) => x.id === c.id)) await insertCommentRow(conn, c);
    }

    for (const tl of after.timeline) {
      if (!before.timeline.some((x) => x.id === tl.id)) await insertTimelineRow(conn, tl);
    }
    ...
    for (const a of after.auditLogs) {
      if (!before.auditLogs.some((x) => x.id === a.id)) await insertAuditRow(conn, a);
    }
