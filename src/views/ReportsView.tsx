...
import { Ticket, TicketStatus, TicketPriority, TicketCategory, Branch, User } from '../types';
import { slaStatusFor, SlaStatus } from '../services/store';
import {
  BarChart3,
...
interface ReportsViewProps {
  tickets: Ticket[];
  branches: Branch[];
  categories: { id: string; name: string; slaHours: number }[];
  users: User[];
}
...
export const ReportsView: React.FC<ReportsViewProps> = ({ tickets, branches }) => {
  ...
  const unassigned = tickets.filter((t) => !t.assignedToId && t.status !== 'Closed' && t.status !== 'Cancelled').length;

  const slaCounts: Record<SlaStatus, number> = { ok: 0, critical: 0, breached: 0, na: 0 };
  for (const t of tickets) slaCounts[slaStatusFor(t)] += 1;
  const slaActive = total - slaCounts.na;
  ...
  const priorityCounts = PRIORITY_ORDER.map((p) => ({ priority: p, count: tickets.filter((t) => t.priority === p).length }));
  const priorityMax = Math.max(1, ...priorityCounts.map((p) => p.count));
  ...
  const exportCsv = () => {
    ...
    const rows = [
      ...
      ['SLA Breached', slaCounts.breached],
      ['SLA Critical', slaCounts.critical],
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 text-white p-6 rounded-2xl border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded border border-blue-700 bg-blue-900/60 text-blue-200 text-xs font-bold uppercase tracking-wider mb-2">
            <BarChart3 className="w-3.5 h-3.5 text-blue-400" />
            <span>Reports & Analytics</span>
          </div>
          <h1 className="text-2xl font-black">IT Service Desk Reports</h1>
          <p className="text-xs text-slate-300 mt-1">
            Ticket volume, SLA performance, and workload analytics across all Bayanihan Bank branches
          </p>
        </div>
        <button
          onClick={exportCsv}
          className="px-4 py-2 bg-blue-700 hover:bg-blue-600 text-white font-bold text-xs rounded-xl transition shadow-sm flex items-center gap-1.5 cursor-pointer self-start md:self-auto"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <div className="bg-white p-4 rounded-xl border border-red-200 bg-red-50/40">
          <span className="text-[10px] font-bold text-red-700 uppercase flex items-center gap-1">
            <AlertTriangle className="w-3 h-3" /> SLA Breached
          </span>
          <div className="text-2xl font-black text-red-800 mt-1">{slaCounts.breached}</div>
        </div>
        <div className="bg-white p-4 rounded-xl border border-slate-200">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 sm:p-5 border-b border-slate-200 bg-slate-50/60">
          <h3 className="text-sm font-bold text-slate-900">SLA Performance</h3>
          <p className="text-xs text-slate-500">Distribution of active tickets by SLA health</p>
        </div>
        <div className="p-4 sm:p-5 grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: 'On Track', value: slaCounts.ok, color: 'text-emerald-700', bg: 'bg-emerald-50 border-emerald-200', bar: 'bg-emerald-600' },
            { label: 'At Risk', value: slaCounts.critical, color: 'text-amber-700', bg: 'bg-amber-50 border-amber-200', bar: 'bg-amber-500' },
            { label: 'Breached', value: slaCounts.breached, color: 'text-red-700', bg: 'bg-red-50 border-red-200', bar: 'bg-red-600' },
            { label: 'Not Applicable', value: slaCounts.na, color: 'text-slate-600', bg: 'bg-slate-50 border-slate-200', bar: 'bg-slate-400' },
          ].map((s) => (
            <div key={s.label} className={`p-4 rounded-xl border ${s.bg}`}>
              ...
              <div className="mt-2 h-1.5 rounded-full bg-white/70 overflow-hidden">
                <div className={`h-full rounded-full ${s.bar}`} style={{ width: `${slaActive > 0 ? Math.max(2, Math.round((s.value / Math.max(1, slaActive)) * 100)) : 0}%` }} />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-4 sm:p-5 border-b border-slate-200 bg-slate-50/60">
            ...
            <p className="text-xs text-slate-500">Severity distribution of all tickets</p>
          </div>
          <div className="p-4 sm:p-5 space-y-3">
            {priorityCounts.map((p) => (
              <div key={p.priority} className="flex items-center gap-3">
                <span className="w-28 shrink-0 text-xs font-semibold text-slate-700">{p.priority}</span>
                <div className="flex-1">
                  <Bar value={p.count} max={priorityMax} className={PRIORITY_COLORS[p.priority]} label={`${p.priority}: ${p.count}`} />
                </div>
          ...
          <div className="flex items-end gap-1.5 h-40">
            {daily.map((d) => (
              <div key={d.date} className="flex-1 flex flex-col items-center gap-1 h-full justify-end">
                ...
                <div
                  className={`w-full rounded-t ${d.count > 0 ? 'bg-blue-600' : 'bg-slate-100'}`}
